    ...
    <div className={`h-[calc(100vh-48px-24px)] flex flex-col ${isDark ? 'bg-slate-900' : 'bg-gradient-to-br from-purple-50 via-blue-50 to-indigo-50'}`}>
      ...
      >
        ...
        <button
          ...
          className="w-11 h-11 rounded-2xl bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center text-white shadow-lg shadow-purple-200 active:scale-95 transition-transform"
        >
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-24">
        ...
        >
          ...
          {error && (
            <div className={`w-full rounded-[28px] p-6 mb-4 ${isDark ? 'bg-amber-900/30 border-amber-700' : 'bg-amber-50 border-amber-200'} border`}>
          ...
          {loading ? (
            <div className={`w-full rounded-[28px] p-6 ${isDark ? 'bg-slate-800' : 'bg-slate-200/50'}`}>
              <p className={isDark ? 'text-slate-500' : 'text-slate-600'}>Loading...</p>
            </div>
          ) : firstCard ? (
            <button 
              onClick={() => onNavigate({ page: 'groupDetail', groupId: firstCard.groupId })}
              className="w-full bg-gradient-to-br from-purple-600 via-indigo-600 to-blue-600 rounded-[28px] p-6 shadow-2xl shadow-purple-300/50 active:scale-[0.98] transition-transform relative overflow-hidden"
            >
              ...
              <div className="relative">
                <div className="flex items-start justify-between mb-10">
                  <div>
                    <p className="text-purple-200 text-xs mb-2 font-medium">Virtual Group Card • {firstCard.groupName}</p>
                    <p className="text-white font-mono text-xl tracking-wide">•••• •••• •••• {firstCard.cardLastFour || '----'}</p>
                  </div>
                ...
                <div className="flex items-end justify-between">
                  ...
                  <div className="bg-white/20 backdrop-blur-xl text-white text-xs font-semibold px-5 py-2.5 rounded-full border border-white/30">
                    View Details
                  </div>
                </div>
              </div>
            ...
            <button 
              ...
              className={`w-full rounded-[28px] p-6 border-2 border-dashed ${isDark ? 'border-slate-600 bg-slate-800/50' : 'border-slate-300 bg-slate-100/50'} active:scale-[0.98] transition-transform`}
            >
        ...
        >
          ...
          <div className="space-y-3">
            ...
            {!loading && !error && groups.length === 0 && (
              <div className={`rounded-[24px] p-8 text-center border ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'}`}>
                <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>No groups yet. Create one to get started.</p>
                <button 
                  onClick={() => onNavigate('createGroup')}
                  className="mt-3 text-purple-600 font-semibold text-sm"
                >
                  Create Group
                </button>
              </div>
            )}
            {!loading && groups.slice(0, 3).map((group, index) => (
              <motion.button
                key={group.id}
                ...
                className={`w-full text-left ${isDark ? 'bg-slate-800' : 'bg-white'} rounded-[24px] p-5 shadow-lg ${isDark ? 'shadow-none' : 'shadow-slate-200/50'} active:scale-[0.98] transition-transform border ${isDark ? 'border-slate-700' : 'border-slate-100'}`}
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className="w-14 h-14 rounded-[18px] bg-gradient-to-br from-purple-400 to-indigo-600 flex items-center justify-center shadow-lg shadow-purple-200/50">
      ...
      >
        <button 
          ...
          className="w-[68px] h-[68px] bg-gradient-to-r from-purple-600 to-indigo-600 rounded-[24px] shadow-2xl shadow-purple-400/50 flex items-center justify-center active:scale-95 transition-transform"
        >
      ...
      <div className={`fixed bottom-0 left-0 right-0 ${isDark ? 'bg-slate-800/95' : 'bg-white/95'} backdrop-blur-2xl border-t ${isDark ? 'border-slate-700' : 'border-slate-200'} z-20`}>
        <div className="mx-auto max-w-[430px] px-6 pt-3 pb-7">
          <div className="flex items-center justify-around">
            ...
            >
              <div className="w-11 h-11 rounded-[16px] bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center">
