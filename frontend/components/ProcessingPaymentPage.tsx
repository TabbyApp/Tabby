        ...
        {step === 'processing' ? (
          ...
          >
            <div className="relative w-32 h-32 mx-auto mb-8">
              <motion.div animate={{ rotate: 360 }} transition={{ duration: 2, repeat: Infinity, ease: 'linear' }} className="w-32 h-32 border-4 border-primary/30 border-t-primary rounded-full" />
              <div className="absolute inset-0 flex items-center justify-center">
                <CreditCard size={40} className="text-primary" />
              </div>
            </div>

            <h2 className="text-2xl font-bold text-foreground mb-3">Processing Payments</h2>
            <p className="text-muted-foreground mb-8">Charging cards for each member...</p>
            ...
            <div className="space-y-3 max-w-sm mx-auto">
              {members.map((member, index) => (
                ...
                >
                  <div className="w-10 h-10 rounded-full bg-primary/20 flex items-center justify-center text-lg">{member.avatar}</div>
                  <div className="flex-1 text-left">
                    <p className="font-semibold text-foreground">{member.name}</p>
                    <p className="text-sm text-muted-foreground">${member.amount.toFixed(2)}</p>
                  </div>
          ...
          <motion.div
            ...
            className="text-center"
          >
            <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 200, damping: 20, delay: 0.1 }} className="w-32 h-32 rounded-full bg-success flex items-center justify-center mx-auto mb-8">
              <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 300, damping: 20, delay: 0.2 }}>
                <Check size={64} className="text-success-foreground" strokeWidth={3} />
              </motion.div>
            </motion.div>

            <motion.h2 initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.25 }} className="text-3xl font-bold text-foreground mb-3">Payment Complete!</motion.h2>
            <motion.p initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3 }} className="text-muted-foreground mb-8">All charges have been processed successfully</motion.p>

            <motion.div initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.35 }} className="bg-card border border-border rounded-2xl p-6 max-w-sm mx-auto">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  <Users size={20} className="text-muted-foreground" />
                  <p className="font-medium text-muted-foreground">{members.length} members charged</p>
                </div>
              ...
              <div className="border-t border-border pt-4 space-y-2">
                {members.map((member) => (
                  <div key={member.id} className="flex items-center justify-between">
                    ...
                    <p className="text-sm font-semibold text-foreground">${member.amount.toFixed(2)}</p>
                  </div>
                ))}
              </div>
            </motion.div>

            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="mt-6 text-sm text-muted-foreground">Returning to home...</motion.p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
